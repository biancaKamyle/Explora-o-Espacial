...
let correnteImagem = 0;

function galeriaProximo() {
 correnteImagem++;
 if(correnteImagem >=max){
   correnteImagem = 0;
 }
 slide.src = servidorDasImagens + '/' + imagens[correnteImagem]['arquivo'];
 slide.alt = imagens[correnteImagem]['descricao'];
...
let botaoAnterio = document.querySelector('#anterior');
botaoAnterio.addEventListener('click', galeriaAnterior );
   
let botaoProximo = document.querySelector('#proximo');
botaoProximo.addEventListener('click', galeriaProximo );
